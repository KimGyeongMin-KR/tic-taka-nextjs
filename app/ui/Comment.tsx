...
const CommentModal = ({ onClose, windowSize, postId, optionIds }: {
  ...
  const commentSubmit = async () => {
    ...
    const commentPostUrl = (reply.id !== null) ? `http://localhost:8000/post/${postId}/comment/${reply.id}/` : `http://localhost:8000/post/${postId}/comment`;
...
const Comment: React.FC<CommentProps> = ({
  ...
  const [nextUrl, setNextUrl] = useState<string|null>(`http://localhost:8000/post/${post_id}/comment/${id}/`);
  ...
  const fetchRepliess = async () => {
    ...
    const response = await fetch(`http://localhost:8000/post/${post_id}/comment/${id}/`, {method: "GET", headers: headers});
    const data = await response.json();
    return data
  }
  ...
  const commentLike = async () => {
    ...
    const response = await fetch(`http://localhost:8000/post/${post_id}/comment/${id}/like`, {method: "POST", headers: headers, body: JSON.stringify(body)});
    if(!response.ok){
      return
    }
    const data = await response.json();
